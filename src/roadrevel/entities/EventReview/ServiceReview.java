...
public class ServiceReview implements IService<Review>{
    ...
    public void ajouter(Review p) {
        String req = "INSERT INTO Review(Event_name ,Review_txt ,id_user) VALUES( ?, ? ,? );";
        try {
    ...
    public void modifier(Review p) {
        String req = "UPDATE  Review SET Event_name=?, Review_txt= ?  , id_user = ?  WHERE Review_id =?";
        try {
            PreparedStatement pst = cnx.prepareStatement(req);
    ...
    public void supprimer(Review p) {
        String req = "DELETE FROM Review WHERE Review_id=?";
        try {
            PreparedStatement pst = cnx.prepareStatement(req);
    ...
    public List<Review> afficher() {
        ...
        try {
            ...
            while(result.next()) {
                list.add(new Review(result.getInt("Review_id"), result.getString("Event_name"),result.getString("Review_txt"),result.getInt("id_user")));
            }
